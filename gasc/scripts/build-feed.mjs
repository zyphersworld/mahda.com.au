...
// Covers both the event name and the venue so quiet weeks still surface news.
const QUERY = '("Grand Annual Sprintcar Classic" OR "Premier Speedway" OR GASC) (sprintcar OR Warrnambool OR Classic OR speedway) when:45d';
...
async function main() {
  ...
  try {
    const res = await fetch(RSS_URL, {
      headers: { 'User-Agent': 'Mozilla/5.0 (feed-bot; the-pitstop)' }
    });
  ...
  await writeFile('gasc/headlines.json', JSON.stringify(payload, null, 2) + '\n', 'utf8');
